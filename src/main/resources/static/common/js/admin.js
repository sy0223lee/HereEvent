...
/***** 대기 페이지 *****/
// 페이지네이션 버튼(전체 페이지 수, 현재 페이지 번호, 전체 조회된 요소의 개수)
function waitPageBtn(totalPages, pageNumber, totalElements) {
    // 보여줄 리스트가 없으면 페이지 버튼 출력 X
    if (totalElements === 0) {
        document.querySelector(".pagination").innerHTML = "";
        return false;
    ...
    if (prevBlockPageNumber >= 0) {
        // <, << 활성화
        strHTML += "<li class='page-item'><button type='button' onclick='moveWaitPage(0)' class='page-btn'><i class='bi bi-chevron-double-left'></i></button></li>";
        strHTML += "<li class='page-item'><button type='button' onclick='moveWaitPage(" + prevBlockPageNumber + ")' class='page-btn'><i class='bi bi-chevron-left'></i></button></li>";
    } else {
        // <, << 비활성화
        strHTML += "<li class='page-item disabled'><button type='button' class='page-btn'><i class='bi bi-chevron-double-left'></button></i></li>";
        strHTML += "<li class='page-item disabled'><button type='button' class='page-btn'><i class='bi bi-chevron-left'></i></button></li>";
    ...
    for (let i = startPageNumber; i <= endPageNumber; i++) {
        if (i === pageNumber) {
            ...
            strHTML += "<li class='page-item'><button type='button' onclick='moveWaitPage(" + i + ")' class='page-btn'>" + (i+1) + "</button></li>";
        }
    }
    ...
    if (nextBlockPageNumber < totalPages) {
        // >, >> 활성화
        strHTML += "<li class='page-item'><button type='button' onclick='moveWaitPage(" + nextBlockPageNumber + ")' class='page-btn'><i class='bi bi-chevron-right'></i></button></li>";
        strHTML += "<li class='page-item'><button type='button' onclick='moveWaitPage(" + (totalPages-1) + ")' class='page-btn'><i class='bi bi-chevron-double-right'></i></button></li>";
    } else {
        // >, >> 비활성화
        strHTML += "<li class='page-item disabled'><button type='button' class='page-btn'><i class='bi bi-chevron-right'></i></button></li>";
        strHTML += "<li class='page-item disabled'><button type='button' class='page-btn'><i class='bi bi-chevron-double-right'></i></button></li>";
...
// 페이지 이동 버튼 - 전달 받은 페이지 번호로 이동
function moveWaitPage(pageNumber){
    // 컨트롤러에게 전해줄 파라미터
    let type = $("select[name='type']").val();
    let keyword = $("#keyword").val();

    // 경로 이동
    location.href = "/hereevent/admin/wait?type=" + type + "&keyword=" + keyword + "&page=" + pageNumber;
}